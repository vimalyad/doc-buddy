...
1. **Query rewrite** — the raw question is rewritten into a retrieval-optimized query (`prompts/queryRewritePrompt.ts`). It is **history-aware**: the frontend sends recent chat turns (`history`), and `askController.normalizeHistory` validates/caps them to the last `MAX_HISTORY_TURNS` (6); the rewrite then condenses follow-ups ("what about its limits?") into a standalone query by resolving references against the history. Silently falls back to the original question on any failure (or when no history is sent).
...
### Frontend
`src/App.tsx` is a two-pane shell: `FileUpload` sidebar (`components/FileUpload.tsx`) and `ChatInterface` (`components/ChatInterface.tsx`). All backend calls go through `axios` against `VITE_API_URL`. File list state lives in `App` and is refreshed via the `onRefresh` callback after uploads/deletes. `ChatInterface` persists messages to `localStorage` and sends the last 6 turns as `history` on each `/api/ask` call to power conversational query rewriting.
